...
function openForm(type: "log" | "reg") {
    if (!type) throw new Error("toggleForm function don't have type parameter.");
    if (type == "log") {
...
formLog?.addEventListener("submit", async (e) => {
    ...
    if (response.ok && response.status == 200) {
        ...
        const user = data.users.find((x: any) => x.login == inputs[0].value);
        if (!user) {
