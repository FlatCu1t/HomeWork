...
(async () => {
    ...
    if (response.ok && response.status == 200) {
        ...
        const userAvatar: any = document.getElementById("userAvatar");
        const userName: any = document.getElementById("userName");
        userAvatar.src = data.avatar;
        userName.textContent = data.userName;

        sendButton?.addEventListener("click", () => {
        const input: any = document.getElementById('messageInput');
...
socket.on('chat message',(msg: any)=>{
    const messagesContainer: any = document.querySelector(".right_section_messages");
...
socket.on('room messages',(msg: any)=>{
