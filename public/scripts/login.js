...
logForm?.addEventListener("submit", async (e) => {
    ...
    if (response.ok && response.status == 200) {
        ...
        const finded = data.users.find((x) => x.name == inputs[0].value || x.email == inputs[0].value);
        ...
        if (!finded)
            return functions.showAlarm(alarm, "error", "Пользователь не найден.");
        if (finded.password !== updatedPassword.password)
            return functions.showAlarm(alarm, "error", "Не правильный пароль.");
...
regForm?.addEventListener("submit", async (e) => {
    ...
    if (response.ok && response.status == 200) {
        const data = await response.json();
        const finded = data.users.find((x) => x.name.toLowerCase() == inputs[0].value.toLowerCase() || x.email.toLowerCase() == inputs[2].value.toLowerCase());
        if (finded)
            return functions.showAlarm(alarm, "error", "Такой пользователь уже существует.");
