...
(async () => {
    ...
    if (response.ok && response.status == 200) {
        ...
        const user = data.users.find((x) => x.id == parseInt(localStorage.getItem("ID") || "0"));
        if (!user) {
            localStorage.clear();
            window.location.assign("/login");
        }
...
changeAvatarBtn?.addEventListener("click", async () => {
    ...
    if (response.ok && response.status == 200) {
        ...
        const user = data.users.find((x) => x.id == parseInt(localStorage.getItem("ID") || "0"));
        if (user) {
            const input = document.getElementById("changeAvatar_input");
            if (!input || input.value.length < 1)
                return alert("Введите URL для аватара.");
            const payload = {
                id: user.id,
                avatar: input.value
            };
